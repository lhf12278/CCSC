...
class Chair(BaseImageDataset):
    ...
    dataset_dir = 'QMUL/Chair'

    def __init__(self, root='', verbose=True, pid_begin=0, relabel=True, **kwargs):
        super(Chair, self).__init__()
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.sketrain_dir = osp.join(self.dataset_dir, 'trainA/')
        self.rgbtrain_dir = osp.join(self.dataset_dir, 'trainB/')
        self.query_dir = osp.join(self.dataset_dir, 'testA/')
        self.gallery_dir = osp.join(self.dataset_dir, 'testB/')
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        for img_path in sorted(img_paths):
            if 'trainA' in img_path or 'testA' in img_path:
                pid = img_path.split('/')[-1].split('.')[0].split('_')[0]
